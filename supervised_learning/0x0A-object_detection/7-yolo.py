...
class Yolo:
    ...
    def predict(self, folder_path):
        """
        Predicts and displays images using the show_boxes method
        Args:
            folder_path: path to folder holding images to predict
        ...
        for i in range(pimages.shape[0]):
            # Grab an output
            outp = [output[i] for output in outputs]
            # Process an output
            boxes, box_confidences, box_class_probs = (
                self.process_outputs(outp, image_shapes[i]))
            # Filter processed outputs
            boxes, box_classes, box_scores = (
                self.filter_boxes(boxes, box_confidences, box_class_probs))
            # Apply non-max suppression
            boxes, box_classes, box_scores = (
                self.non_max_suppression(boxes, box_classes, box_scores))
            # File names
